...
function Menu1Settings() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if(res?.success) {
        const blocks = res.data.filter((block) => block.categoryBlockType == "MENU_1")
        console.log(blocks);
