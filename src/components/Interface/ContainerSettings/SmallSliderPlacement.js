...
import {
  editCategoryBlockById,
  getAllCategoryBLocksList,
} from "../../../api/CategoryBlocks";
import MyContext from "../../Context/MyContext";
import SliderSortableItems from "./SliderSortableItems";
import { editSmallSliderById, getAllSmallSliderList } from "../../../api/SmallSliderPlacement";

function SmallSliderPlacement() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const tempBoxes = [];
      const res = await getAllSmallSliderList();
      if (res?.success) {
        console.log(res.data);
        

        res.data?.map((b, index) => {
          tempBoxes.push({ id: index + 1, containerBlock: b });
        });
        setBoxes(tempBoxes);
      }
    };
  ...
  const editBlockPlacment = () => {
    ...
    newBloks.map((blockItem, index) => {
      ...
      const fetchData = async () => {
        const res = await editSmallSliderById(blockItem.id, data);
        if (res?.success) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4">
      <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        ...
        >
          <div className="flex flex-wrap gap-2 border p-4 rounded-lg bg-gray-100">
            {boxes.map((box, index) => (
              <Box key={`SmallSlider_${box.id}`}>
                <SliderSortableItems
...
export default SmallSliderPlacement;
