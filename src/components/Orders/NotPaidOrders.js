...
function NotPaidOrders({ notPaidOrders }) {
  const navigate = useNavigate();

  let sum = notPaidOrders?.reduce((acc, item) => acc + item.orderCost, 0);
    ...
    <Box>
      <Card sx={{ padding: 2, maxWidth: "500px", marginTop: 7 }}>
        <Box display={"flex"} gap={2}>
          ...
          <Typography>{notPaidOrders?.length}</Typography>
        </Box>
        <Box display={"flex"} gap={2}>
          <Typography sx={{ color: "grey" }}>
