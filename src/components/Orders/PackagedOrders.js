...
function PackagedOrders({ packagedOrders }) {
  const navigate = useNavigate();

  let sum = packagedOrders?.reduce((acc, item) => acc + item.orderCost, 0);
    ...
    <Box>
      <Card sx={{ padding: 2, maxWidth: "500px", marginTop: 7 }}>
        <Box display={"flex"} gap={2}>
          ...
          <Typography>{packagedOrders?.length}</Typography>
        </Box>
        <Box display={"flex"} gap={2}>
          <Typography sx={{ color: "grey" }}>
            Yangi buyurtmalarning umumiy qiymati:
          </Typography>
          <Typography>{sum.toLocaleString().replace(/,/g, " ")} söm</Typography>
        </Box>
      </Card>
        ...
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          <TableHead>
            <TableRow sx={{ backgroundColor: "#f5f5f5" }}>
              <TableCell
                sx={{ maxWidth: "20px", borderRight: "1px solid grey" }}
              >
                #
              </TableCell>
              ...
              <TableCell align="center">Buyurtma №</TableCell>
          ...
          <TableBody>
            {packagedOrders?.map((row, idx) => {
              ...
              return (
                ...
                >
                  <TableCell
                    sx={{ maxWidth: 20, borderRight: "1px solid grey" }}
                    component="th"
                    scope="row"
                  >
                    {idx + 1}
